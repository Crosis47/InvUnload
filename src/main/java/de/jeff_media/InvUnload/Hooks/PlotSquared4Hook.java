...
public class PlotSquared4Hook implements PlotSquaredUniversalHook {
	...
	public PlotSquared4Hook() {
		try {
			Class<?> bukkitUtilClass = Class.forName("com.github.intellectualsites.plotsquared.bukkit.util.BukkitUtil");
			...
			throw new IllegalStateException("Could not initialize PlotSquared v4 hook", e);
		}
	}
	...
	public boolean isBlockedByPlotSquared(Block block, Player player, Main main) {
		try {
			...
			main.getLogger().warning("Could not query PlotSquared v4 hook.");
			return false;
		}
	}
